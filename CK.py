...
class CK(data.Dataset):
    ...
    def __init__(self, split='Training', fold=1, transform=None):
        ...
        sum_number = [0]  # [0, 135, 312, 387, 594, 678, 927, 981]  # the sum of class number
        test_number = []  # [12, 18, 9, 21, 9, 24, 6]  # the number of each class
        for i in range(1, number):
            if self.data['data_label'][i] != self.data['data_label'][i - 1]:
                sum_number.append(i)
                test_number.append(int(math.ceil(float((i - sum_number[-2]) / 5))))
        sum_number.append(number)
        test_number.append(int(math.ceil(float((number - sum_number[-2]) / 5))))
